...
/*
PlatformCollector extends the prometheus.Collector interface to allow customization of the metric
...
/*
lazyPlatformMetric defines our metric registration interface. lazyMetric objects are expected
...
// lazyInit provides the lazyMetric with a reference to the PlatformCollector it is supposed
// to allow lazy initialization for. It should be invoked in the factory function which creates new
// PlatformCollector type objects.
func (r *lazyMetric) lazyInit(self PlatformCollector, fqName string) {
	r.fqName = fqName
	r.self = self
}
...
func (r *lazyMetric) determineDeprecationStatus(version semver.Version) {
	...
	r.markDeprecationOnce.Do(func() {
		...
		if ShouldShowHidden() {
			// TODO : instantiate package specific logging
		...
		if shouldHide(&version, selfVersion) {
			// log.Warningf("This metric has been deprecated for more than one release, hiding.")
			r.isHidden = true
		}
	})
}
