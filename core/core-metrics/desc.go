...
// Desc is a promethus.Desc extension
//
...
type Desc struct {
	...
	help string
	// constLabels is the label names. Their label values are variables
	constLabels Labels
	// variableLabels contains names of labels for which the metrics maintains variable values
	...
	// deprecatedVersion represents in which version this metric is deprecated
...
func (d *Desc) determineDeprecationStatus(version semver.Version) {
	...
	d.markDeprecationOnce.Do(func() {
		...
		if ShouldShowHidden() {
			// TODO: use core library loggin instance to log output
			// log.Warningf("Hidden metrics(%s) have been manually overridden, showing this very deprecated metric.", d.fqName)
			return
		}
		if shouldHide(&version, selfVersion) {
			// log.Warningf("This metric(%s) has been deprecated for more than one release, hiding.", d.fqName)
			d.isHidden = true
		}
	})
}
